...
const InstitutionCard: React.FC<InstitutionCardsProps> = ({ institution }) => {
  const { user } = useAuth();
      ...
      <section className="relative h-[136px] lg:fill-two-columns fill-column mb-[15px] border border-gray-300 rounded-sm overflow-hidden bg-[#FFFFFF] shadow-[0_0_3px_#AABBCC] hover:shadow-[0_0_10px_#AABBCC]">
        ...
        >
            ...
            <Image
              ...
              onLoad={() =>
                setLoadedImages((prev) => ({
                  ...prev,
                  [institution.imageUrl]: true,
                }))
              }
              style={
                loadedImages[institution.imageUrl]
                  ? {}
                  : {
                      backgroundImage:
          ...
          <section className="w-full flex flex-col justify-between p-[15px]">
            <div className="xl:w-[380px] xs:w-[300px] xxs:w-[168px] w-[100px] common-card pr-[15px] text-[16px] text-[#3E3A39] font-bold">
              {institution.hosp_name}
            </div>
            <div className="xl:w-[380px] xs:w-[300px] xxs:w-[168px] w-[100px] common-card text-[14px] text-[#595959]">
              {institution.division}
            </div>
            <div className="xl:w-[380px] xs:w-[300px] xxs:w-[168px] w-[100px] common-card text-[14px] text-[#595959]">
              {institution.cancer_screening}
            </div>
            <div className="xl:w-[380px] xs:w-[300px] xxs:w-[168px] w-[100px]  h-[30px] common-row-flex">
              ...
              />
              <span className="ml-[5px] text-[14px] text-[#707070]">
                {institution.view}
              </span>
            </div>
          </section>
        ...
        {!user ? (
          ...
          <>
            {(() => {
              const isFavorited = (institution: InstitutionInfo) =>
                state.favorites.some(
                  (item) =>
                    item.userId === user.uid &&
                    item.hosp_name === institution.hosp_name
                );
              const handleFavoriteClick = isFavorited(institution)
                ? () => handleRemoveFavorite(user, institution.hosp_name)
                : () => handleAddFavorite(user, institution);
              ...
              return (
                ...
                >
                  <Image
                    src={
                      isFavorited(institution)
                        ? "/images/diamond_selected.png"
