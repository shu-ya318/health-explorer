interface CancerFilterProps {
  handleDebouncedCancerFilter: (filter: string) => void;
}
      ...
      <div className="grid md:grid-cols-5 grid-cols-3 md:gap-x-16 xs:gap-x-20 xxs:gap-x-12 gap-x-10">
        {cancers.map((cancer, index) => (
          ...
          >
            ...
            ></div>
            <div className="w-full mb-[10px] xs:text-[20px] text-[14px] text-[#252525] text-center font-bold">
              {cancer.filter}
            </div>
          </button>
        ))}
