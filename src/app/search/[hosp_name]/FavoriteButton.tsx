          ...
          <>
            {(() => {
              const isFavorited = state.favorites.some(
                (item) =>
                  item.userId === user.uid && item.hosp_name === institutionName
              );
              const handleFavoriteClick = isFavorited
                ? () => handleRemoveFavorite(user, institutionName)
                : () => handleAddFavorite(user, institutionDetails);
              return (
                <button
                  onClick={() => {
                    handleFavoriteClick();
