...
const InstitutionPage: React.FC = () => {
  ...
  const [comparableInstitutions, setComparableInstitutions] = useState<
    InstitutionInfo[]
  >([]);
    ...
    const fetchComparableInstitutions = async (): Promise<void> => {
      ...
      try {
        ...
        const filteredHits = hits
          .filter((hit) => hit.objectID !== institutionDetails.objectID)
          .map((hit) => ({
            ...
            <main className="common-col-flex justify-center w-full h-auto bg-[#FCFCFC]">
              ...
              <section className="common-col-flex xl:w-full max-w-[1180px] w-[95%] mt-[100px] my-[50px] bg-[#FFFFFF] common-border border-2 shadow-[0_0_5px_#AABBCC]">
                <div className="w-full xs:px-[30px] px-[10px]">
                  ...
                  <h3 className="xs:institutionPage-title-xs institutionPage-title-mobile mb-[30px]">
                    資訊簡介
                  </h3>
                  <div className="w-full h-full flex flex-col sm:justify-around justify-center text-black sm:text-xl">
                    <div className="w-full flex mb-[25px]">
                      <span className="w-[90px] flex-shrink-0 font-bold ">
                        名稱
                      </span>
                      <span>{institutionDetails.hosp_name}</span>
                    </div>
                    <div className="w-full flex mb-[25px]">
                      <span className="w-[90px] flex-shrink-0 font-bold ">
                        電話
                  ...
                  <h3 className="xs:institutionPage-title-xs institutionPage-title-mobile  mb-[30px] mt-[5px]">
                    地圖實景
                  </h3>
                  {loading ? (
                    <div className="w-[90%] h-[450px] my-[40px] bg-gray-300 rounded-lg animate-pulse"></div>
                  ) : (
                    <InstitutionMap institutionDetails={institutionDetails} />
                  )}
