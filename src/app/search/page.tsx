...
const SearchPage: React.FC = () => {
    ...
    const fetchAndSetData = async (): Promise<void> => {
      ...
      try {
        if (filterValue) {
          ...
          const response = await index.search<InstitutionInfo>(
            filterValue,
            searchOptions
          );
  ...
  //  只寫setTimeout會提醒Type "timeout" is not assignable to type "number"
  const handleDebouncedCancerFilter = useCallback((cancerType: string) => {
    setLoading(true);
    ...
    debounceTimeoutID.current = window.setTimeout(async () => {
      try {
        ...
        const { hits } = await index.search<InstitutionInfo>(
          cancerType,
          searchOptions
        );
        setCurrentData(hits);
  ...
  const toggleDropdowns = (
    type: "institutions" | "divisions" | "districts"
  ): void => {
    setIsOpenInstitutions(
      type === "institutions" ? !isOpenInstitutions : false
    );
  ...
  return (
    <main className="w-full h-auto common-col-flex justify-center bg-[#FCFCFC]">
      <div className="w-full h-auto relative flex">
        ...
        <CancerFilter
          handleDebouncedCancerFilter={handleDebouncedCancerFilter}
        />
      </div>
        ...
        <section className="w-full h-auto flex flex-col items-start">
          {loading ? (
            ...
            <p className="text-[#595959] text-left">
              共有<strong className="mx-[6px]">{currentData.length}</strong>
              個新北市醫療機構
            </p>
          )}
