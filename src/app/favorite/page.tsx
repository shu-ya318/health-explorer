  ...
  const handleConfirmDelete = async (hosp_name: string): Promise<void> => {
    ...
    try {
      ...
      setFavoriteData((prevData) =>
        prevData.filter((item) => item.hosp_name !== hosp_name)
      );
    } catch (error) {
      console.error(error);
    } finally {
                ...
                <main className="w-full h-auto common-col-flex justify-center">
                  {selectedId && (
                    <ConfirmDeleteModal
                      ...
                      hosp_name={
                        favoriteData.find((item) => item.id === selectedId)
                          ?.hosp_name || ""
                      }
                    />
                  )}
                  <section className="relative w-full h-auto flex">
                    <div className="relative w-full h-[360px] flex flex-col">
                      ...
                      <div className="absolute inset-0 w-full h-full bg-gray-900 bg-opacity-20">
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-bold text-[#FFFFFF] text-center sm:text-[56px] xs:text-[48px] text-[37px]">
                          收藏清單
                        </div>
                      </div>
                    </div>
