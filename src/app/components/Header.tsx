...
const Header: React.FC = () => {
  ...
  const [isLogOutToastVisible, setIsLogOutToastVisible] =
      ...
      if (typeof window !== "undefined" && typeof MouseEvent !== "undefined") {
        if (event instanceof MouseEvent) {
          ...
          if (
            isMenuBarVisible &&
            menuDivRef.current &&
            !menuDivRef.current.contains(target)
          ) {
            if (isMenuBarVisible) {
      ...
      <header className="fixed inset-x-0 top-0 z-30 w-screen h-[60px] common-col-flex justify-between bg-[#FFFFFF] border border-solid border-[#9FC5DF]">
        ...
        <section className="relative max-w-[1200px] w-screen lg:w-[90%] md:w-[80%] common-row-flex justify-between m-auto px-[10px] text-[#1E94B4]">
          ...
          {isLogOutToastVisible && logOutMessage && (
            ...
            >
              <div className="my-auto text-[#2D759E] text-center xs:text-[14px] text-[12px]">
                {logOutMessage}
              </div>
              <button
                onClick={closeToast}
          ...
          <div
            className={`md:hidden  
                        ${
                          isMenuBarVisible
                            ? "absolute fixed inset-0 z-40 w-screen h-screen flex bg-black bg-opacity-30"
                            : "hidden"
                        }`}
          ></div>
          <div
            ...
            className={`md:common-row-flex md:justify-between no-underline 
                        ${
                          isMenuBarVisible
                            ? "absolute top-0 right-0 z-50 w-44 h-screen common-col-flex pt-20 bg-[white] shadow-[0_0_3px_#1e94b4]"
                            : "hidden"
                        }`}
          >
            {!user ? (
              <>
                ...
                >
                  <span className="m-auto text-[#2598B6] text-center">
                    登入
                  </span>
                </button>
                ...
                >
                  <span className="m-auto text-[#2598B6] text-center hover:text-[#2D759E]">
                    註冊
                  </span>
                </button>
