...
// (三)放到父函式
export async function setupInstitutionData() {
    /*const snapshot = await getDocs(collection(db, 'medicalInstitutions'));
    if (await checkMultipleEntries()) {
        console.log("Firestore中資料筆數超過一筆，不執行 call API!");
        return;*/
    const snapshot = await getDocs(collection(db, 'medicalInstitutions'));
    if (snapshot.size > 1) {
        console.log("Firestore已初始化整體資料，不執行 call API!");
        return;
    } 
...
/*export  const InstitutionsContext = createContext<FirebaseInstitutionData>();
...
export const AuthContextProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    return (
        <InstitutionsContext.Provider value={institutionData}>
          {children}
