...
const cervicalCancerData: FirebaseInstitutionData[] = [
  ...
  },
  {
    ...
    area: "板橋區",
    hosp_addr:
      "板橋院區:新北市板橋區忠孝路15號 海山院區:新北市土城區中華路一段32號",
  },
  {
...
const formatFunctions: {
  [key: string]: (item: ApiDataItem) => FirebaseInstitutionData;
} = {
  衛生所: (item: ApiDataItem) => ({
    hosp_name: `新北市${item.hosp_name?.substring(0, 3) ?? ""}${
      item.hosp_name?.substring(3) ?? ""
    }`,
    tel: item.tel ?? "",
    area: item.district ?? "",
...
const apiUrls = [
  {
    url: "https://data.ntpc.gov.tw/api/datasets/2553bb1a-bcbb-4284-8b24-acfefe966f1e/json",
    key: "衛生所",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/85bfcaa8-9932-4d06-a2ec-731171191883/json",
    key: "醫院",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/d041976e-de7a-473a-8667-daa9833d777a/json",
    key: "兒科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/77ae6d31-3a9c-49ec-9299-bcd4e19571ef/json",
    key: "婦產科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/248e3ecc-975f-494f-8fdb-1d1a177b8c72/json",
    key: "牙醫一般科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/44fc194a-9964-4f5a-ac0d-de7cf49660ce/json",
    key: "耳鼻喉科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/407c317b-3253-45cc-8755-89c1480e5a55/json",
    key: "皮膚科",
  },
  {
    url: "https://data.ntpc.gov/tw/api/datasets/48a4da9c-3a1b-4247-afde-47abb1ed6a4d/json",
    key: "眼科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/1c770555-c917-4d96-9e6e-9601c496b8ed/json",
    key: "骨科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/6556714e-4eb1-4645-a6de-7f2402527a91/json",
    key: "精神科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/f86e3560-7922-4f0f-b5ee-9ecccf80e2e6/json",
    key: "心理諮商及心理治療科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/9e1c1aba-4e0b-4d5a-8755-efa1f09abe65/json",
    key: "家庭醫學科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/9e36f533-38e9-49fe-99fe-b711cfcbe4c5/json",
    key: "泌尿科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/e8244d31-e0e5-459d-87ba-3c188706fbee/json",
    key: "內科",
  },
  {
    url: "https://data.ntpc.gov/tw/api/datasets/e922e439-a892-4bd3-b756-5a9afb3812b6/json",
    key: "外科",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/08e4e0e8-c32e-4ef7-a262-68ab9debdce7/json",
    key: "子宮頸癌",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/85eeec59-b584-4d97-ba83-8786eb2536a4/json",
    key: "大腸癌",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/4aa54765-a471-4412-a314-468f892a1794/json",
    key: "口腔癌",
  },
  {
    url: "https://data.ntpc.gov.tw/api/datasets/435dae21-f8b9-449b-b9bd-0283a9541f68/json",
    key: "乳癌",
  },
];

function addManualFields(
  item: FirebaseInstitutionData,
  apiKey: string
): FirebaseInstitutionData {
  switch (apiKey) {
    case "衛生所":
      return {
        ...item,
...
async function fetchGeocode(
  ...
  const formattedAddress = formatAddress(item.hosp_addr)
    .replace(/,/g, "")
    .replace(/\s/g, "%20");
...
async function fetchAndFormatData(): Promise<FirebaseInstitutionData[]> {
  ...
  const fetchDataFunctions: ApiFunction[] = apiUrls.map(
    ({ url, key }, index) =>
      async () => {
        try {
          ...
          return data.map((item) => {
            let formattedItem: FirebaseInstitutionData =
              formatFunctions[key](item);
  ...
  const throttledFetchResults: FirebaseInstitutionData[][] =
    await throttlePromises(fetchDataFunctions, 10, 100);
  const results = await Promise.allSettled(throttledFetchResults);
  results.forEach((result, index) => {
    if (result.status === "fulfilled") {
      result.value.forEach((item: FirebaseInstitutionData) => {
        const existingEntry = institutionData.find(
          (entry) => entry.hosp_name === item.hosp_name
        );

        if (
          ["子宮頸癌", "大腸癌", "口腔癌", "乳癌"].includes(apiUrls[index].key)
        ) {
          if (existingEntry) {
            existingEntry.cancer_screening = existingEntry.cancer_screening
              ? `${existingEntry.cancer_screening}, ${apiUrls[index].key}`
              : apiUrls[index].key;
          } else {
          ...
          if (existingEntry) {
            existingEntry.division = existingEntry.division
              ? `${existingEntry.division}, ${apiUrls[index].key}`
              : apiUrls[index].key;
          } else {
            institutionData.push(item);
          }
      ...
      console.error(
        `Error with API URL ${apiUrls[index].url}: ${result.reason}`
      );
    }
  });
  ...
  cervicalCancerData.forEach((item) => {
    ...
    if (existingEntry) {
      existingEntry.cancer_screening = existingEntry.cancer_screening
        ? `${existingEntry.cancer_screening}, 肺癌`
        : "肺癌";
    } else {
      institutionData.push(item);
    }
  ...
  async function processGeocodeAndMapImages(
    data: FirebaseInstitutionData[],
...
async function throttlePromises(
  ...
  const enqueue = function (): Promise<void> {
    if (executing.length >= limit || funcs.length === 0)
  ...
  await Promise.resolve().then(() =>
    Promise.all(Array.from({ length: limit }, enqueue))
  );
